...
try:
    ...
    def TemplateResponse(request, template, context=None, mimetype=None,
...
class rendered_with(object):
    ...
    def __call__(self, func):
        def rendered_func(request, *args, **kwargs):
            ...
            items.setdefault('controller_name', '%s/%s' % (
                func.__module__, func.__name__))
            return TemplateResponse(request, self.template_name,
                                    items, content_type=self.mimetype)
...
def register_admin(model, exclude=[], also=[]):
    ...
    class Admin(admin.ModelAdmin):
        list_display = [
            f.name for f in model._meta.fields if f.name not in exclude
        ] + also
